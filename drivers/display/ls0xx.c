 ...
 *
 * Vendored copy of the upstream Sharp LS0XX driver, including the
 * `serial-vcom-inversion` software VCOM-toggling support added to
 * Zephyr main in commit ac270df99a (2025-09-22). ZMK 0.4 enables that
 * DT property on the nice_view shield as the documented fix for the
 * DC-bias flicker seen since the Zephyr 4.1 bump (PR #3294), but the
 * Zephyr 4.1 in-tree driver does not implement it, leaving the LCD's
 * VCOM bias uninverted in practice.
 *
 * Adaptations vs upstream main:
 *  - SPI_DT_SPEC_INST_GET takes a third `delay` argument in 4.1.
 *  - The display driver API does not have a `.clear` slot in 4.1, so
...
#define LS0XX_VCOM_PRIO       CONFIG_ZMK_SOFLE_LS0XX_VCOM_THREAD_PRIO
...
static const struct ls0xx_config ls0xx_config = {
	/* Zephyr 4.1: SPI_DT_SPEC_INST_GET takes (idx, operation, delay). */
